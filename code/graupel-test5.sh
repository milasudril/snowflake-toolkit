...
for k in {1..4}; do
	...
	cmd="__targets_rel/graupel_generate2 --seed=$seedval --E_0=12 --decay-distance=50 --overlap-max=3 \
		--scale=[1,0.25] --merge-offset=0.2 --D_max=200 --fill-ratio=0 --dump-stats=graupel-output/12-50-$k.txt \
		--dump-geometry-ice=graupel-output/12-50-$k.graupel > graupel-output/12-50-params-$k.txt;bash"
