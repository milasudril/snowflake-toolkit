...
% terminal. If exefile is ommited, the function will start snowflake_simulate3
	...
	statefile_in=ternary(@()( isfield(paramstruct,'statefile_in') && ~isempty(parmastruct.statefile_in) )...
		,@()['--statefile-in=',paramstruct.statefile_in]...
		,@()'');

	statefile_out=ternary(@()( isfield(paramstruct,'statefile_out') && ~isempty(parmastruct.statefile_out) )...
		,@()['--statefile-out=',paramstruct.statefile_out]...
		,@()'');

	stop_cond=ternary(@()( isfield(paramstruct,'stop_cond') && ~isempty(parmastruct.stop_cond) )...
		,@()['--stop-cond=',paramstruct.stop_cond]...
		,@()'');

	shape=ternary(@()( isfield(paramstruct,'shape') && ~isempty(parmastruct.shape) )...
		,@()['--shape=',paramstruct.shape]...
		,@()'');

	deformations={};
	if isfield(paramstruct,'deformations') && ~isempty(parmastruct.deformations)
	...
	prototype_choices='';
	if isfield(paramstruct,'prototype_choices') && ~isempty(parmastruct.prototype_choices)
	...
	output_dir=ternary(@()( isfield(paramstruct,'output_dir') && ~isempty(parmastruct.output_dir) )...
		,@()['--output-dir=',paramstruct.output_dir]...
		,@()'');

	dump_stats=ternary(@()( isfield(paramstruct,'dump_stats') && ~isempty(parmastruct.dump_stats) )...
		,@()['--dump-stats=',num2str(paramstruct.dump_stats)]...
		,@()'');

	dump_geometry=ternary(@()( isfield(paramstruct,'dump_geometry') && ~isempty(parmastruct.dump_geometry) )...
		,@()makebool('--dump-geometry',paramstruct.dump_geometry)...
		,@()'');

	dump_geometry_ice=ternary(@()( isfield(paramstruct,'dump_geometry_ice') && ~isempty(parmastruct.dump_geometry_ice) )...
		,@()makebool('--dump-geometry-ice',paramstruct.dump_geometry_ice)...
		,@()'');

	sample_geometry=ternary(@()( isfield(paramstruct,'sample_geometry') && ~isempty(parmastruct.sample_geometry) )...
	...
	seed=ternary(@()( isfield(paramstruct,'seed') && ~isempty(parmastruct.seed) )...
		,@()['--seed=',int2str(paramstruct.seed)]...
		,@()'');

	N=ternary(@()( isfield(paramstruct,'N') && ~isempty(parmastruct.N) )...
		,@()['--N=',num2str(paramstruct.N)]...
		,@()'');

	droprate=ternary(@()( isfield(paramstruct,'droprate') && ~isempty(parmastruct.droprate) )...
		,@()['--droprate=',num2str(paramstruct.droprate)]...
		,@()'');

	meltrate=ternary(@()( isfield(paramstruct,'meltrate') && ~isempty(parmastruct.meltrate) )...
		,@()['--meltrate=',num2str(paramstruct.meltrate)]...
		,@()'');

	growthrate=ternary(@()( isfield(paramstruct,'growthrate') && ~isempty(parmastruct.growthrate) )...
		,@()['--growthrate=',num2str(paramstruct.growthrate)]...
		,@()'');

	merge_retries=ternary(@()( isfield(paramstruct,'merge_retries') && ~isempty(parmastruct.merge_retries) )...
		,@()['--merge-retries=',num2str(paramstruct.merge_retries)]...
		,@()'');

	overlap_min=ternary(@()( isfield(paramstruct,'overlap_min') && ~isempty(parmastruct.overlap_min) )...
		,@()['--overlap-min=',num2str(paramstruct.overlap_min)]...
		,@()'');

	overlap_max=ternary(@()( isfield(paramstruct,'overlap_max') && ~isempty(parmastruct.overlap_max)  )...
