...
% terminal. If exefile is ommited, the function will start snowflake_simulate3
	...
	stop_cond=ternary(@()( isfield(paramstruct,'stop_cond') && ~isempty(paramstruct.stop_cond) )...
		,@()['--stop-cond=',paramstruct.stop_cond]...
		,@()'');

	shape=ternary(@()( isfield(paramstruct,'shape') && ~isempty(paramstruct.shape) )...
		,@()['--shape=',paramstruct.shape]...
		,@()'');

	deformations={};
	if isfield(paramstruct,'deformations') && ~isempty(paramstruct.deformations)
	...
	prototype_choices='';
	if isfield(paramstruct,'prototype_choices') && ~isempty(paramstruct.prototype_choices)
	...
	output_dir=ternary(@()( isfield(paramstruct,'output_dir') && ~isempty(paramstruct.output_dir) )...
	...
	sample_geometry=ternary(@()( isfield(paramstruct,'sample_geometry') && ~isempty(paramstruct.sample_geometry) )...
	...
	N=ternary(@()( isfield(paramstruct,'N') && ~isempty(paramstruct.N) )...
		,@()['--N=',num2str(paramstruct.N)]...
		,@()'');

	droprate=ternary(@()( isfield(paramstruct,'droprate') && ~isempty(paramstruct.droprate) )...
		,@()['--droprate=',num2str(paramstruct.droprate)]...
		,@()'');

	meltrate=ternary(@()( isfield(paramstruct,'meltrate') && ~isempty(paramstruct.meltrate) )...
		,@()['--meltrate=',num2str(paramstruct.meltrate)]...
		,@()'');

	growthrate=ternary(@()( isfield(paramstruct,'growthrate') && ~isempty(paramstruct.growthrate) )...
		,@()['--growthrate=',num2str(paramstruct.growthrate)]...
		,@()'');

	merge_retries=ternary(@()( isfield(paramstruct,'merge_retries') && ~isempty(paramstruct.merge_retries) )...
		,@()['--merge-retries=',num2str(paramstruct.merge_retries)]...
		,@()'');

	overlap_min=ternary(@()( isfield(paramstruct,'overlap_min') && ~isempty(paramstruct.overlap_min) )...
	...
	overlap_max=ternary(@()( isfield(paramstruct,'overlap_max') && ~isempty(paramstruct.overlap_max)  )...
		,@()['--overlap-max=',num2str(paramstruct.overlap_max)]...
		,@()'');
